...
DESTINATION_INDEX      = 54     # Setting a Destination HERE
...
C4_TRAFFIC_SIGN_FENCELENGTH = 5           # m
C4_PEDESTRIAN_FENCELENGTH = 5             # m
C4_VEHICLE_FENCELENGTH = 5                # m
...
def check_for_obstacles(sensor_data): 
    if sensor_data.get("CameraRGB", None) is not None:
        ...
        if (image_RGB.shape) > (300, 300):
            ...
            blob = cv2.dnn.blobFromImage(cropped_frame, 1/255, (yoloNet.inpWidth, yoloNet.inpHeight), [0,0,0], 1, crop = False)
        else:
            blob = cv2.dnn.blobFromImage(image_RGB, 1/255, (yoloNet.inpWidth, yoloNet.inpHeight), [0,0,0], 1, crop = False)

        # set the net
        yoloNet.net.setInput(blob)
        outs = yoloNet.net.forward(yoloNet.getOutputsNames(yoloNet.net))
        box = yoloNet.postprocess(image_RGB, outs)

        # show the image
        cv2.imshow(yoloNet.winName, image_RGB)

        return box
    return None

def compute_depth(segmentation_data, depth_data, box):
    ...
    xmin-=xmin*percentage
    ymin-=ymin*percentage
    xmax+=xmax*percentage
    ymax+=ymax*percentage
    
    top_left_x = int(xmin)
    bottom_right_x = int(xmax)
    top_left_y = int(ymin)
    bottom_right_y = int(ymax)
    ...
    if found:
        depth = depth_data[j][i] * 1000  # Consider depth in meters
    else:
        depth=1000
    return depth
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        # convert traffic lights' yaw to rad
        for i in range(len(traffic_lights)):
            traffic_lights[i][3] = traffic_lights[i][3] * np.pi / 180.0
        
        trafficlights_fences = []
        thr_deg = 0.5
        # obtain traffic sign fence points for LP
        for i in range(len(traffic_lights)):
            x = traffic_lights[i][0]
            y = traffic_lights[i][1]
            z = traffic_lights[i][2]

            # Choosing properly traffic light's yaw
            yaw_deg = traffic_lights[i][3] * 180.0 / np.pi
            if (abs(yaw_deg) >= 90 - thr_deg) and (abs(yaw_deg) <= 90 + thr_deg):
                yaw = traffic_lights[i][3] + np.pi / 2.0  # add 90 degrees for fence
            elif    ((abs(yaw_deg) >= 0 - thr_deg) and (abs(yaw_deg) <= 0 + thr_deg)) or \
                    ((abs(yaw_deg) >= 180 - thr_deg) and (abs(yaw_deg) <= 180 + thr_deg)):
                yaw = traffic_lights[i][3] - np.pi / 2.0  # remove 90 degrees for fence
            
            spos = np.array([
                    [0, 0                       ],
                    [0, C4_TRAFFIC_SIGN_FENCELENGTH]])
            rotyaw = np.array([
                    [np.cos(yaw), np.sin(yaw)],
                    [-np.sin(yaw), np.cos(yaw)]])
            spos_shift = np.array([
            ...
            trafficlights_fences.append([spos[0,0], spos[1,0], spos[0,1], spos[1,1], traffic_lights[i][4]])

        bp.set_trafficlights_fences(trafficlights_fences)
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            bp.set_measurement_non_player_agents(measurement_data.non_player_agents)
            ...
            # Handling obstacles
            #############################################
            # convert obstacles' yaw to rad
            for i in range(len(obstacles)):
                obstacles[i][3] = obstacles[i][3] * np.pi / 180.0
            ...
            #############################################
            # Handling pedestrians
            #############################################
            # convert pedestrians' yaw to rad
            for i in range(len(pedestrians)):
                pedestrians[i][3] = pedestrians[i][3] * np.pi / 180.0 
            
            pedestrians_fences = []
            stay_stopped = bp._state == behavioural_planner.STAY_STOPPED
            why_decelerate = bp._why_decelerate
            # obtain pedestrian fence points for LP
            for i in range(len(pedestrians)):
                x = pedestrians[i][0]
                y = pedestrians[i][1]
                z = pedestrians[i][2]
                yaw = pedestrians[i][3] + np.pi / 2.0  # add 90 degrees for fence
                spos = np.array([
                        [0, 0                       ],
                        [0, C4_PEDESTRIAN_FENCELENGTH]])
                rotyaw = np.array([
                        [np.cos(yaw), np.sin(yaw)],
                        [-np.sin(yaw), np.cos(yaw)]])
                spos_shift = np.array([
                ...
                # When we stopped because of pedestrians on the road, we check if
                # they are effectivelly obstructing our lane or not. If not, we move on.
                if stay_stopped and why_decelerate == 2:
                    # Compute the angle between the normalized vector between the pedestrian
                    # and ego vehicle position with the ego vehicle's heading vector.
                    pedestrian_delta_vector = [pedestrians[i][0] - current_x, 
                                                pedestrians[i][1] - current_y]
                    pedestrian_distance = np.linalg.norm(pedestrian_delta_vector)
                    # In this case, the pedestrian is in the range of interest.
                    if pedestrian_distance < 20:
                        pedestrian_delta_vector = np.divide(pedestrian_delta_vector, 
                                                        pedestrian_distance)
                        ego_heading_vector = [math.cos(current_yaw), 
                        ...
                        if np.dot(pedestrian_delta_vector, 
                                ego_heading_vector) < ((math.sqrt(6) + math.sqrt(2))/4):
                            continue
                        pedestrians_fences.append([spos[0,0], spos[1,0], spos[0,1], spos[1,1], pedestrians[i][4]])
                else:
                    pedestrians_fences.append([spos[0,0], spos[1,0], spos[0,1], spos[1,1], pedestrians[i][4]])
            
            bp.set_pedestrians_fences(pedestrians_fences)

            #############################################
            # Handling vehicles
            #############################################
            # convert vehicles' yaw to rad
            for i in range(len(vehicles)):
                vehicles[i][3] = vehicles[i][3] * np.pi / 180.0 
            
            vehicles_fences = []
            stay_stopped = bp._state == behavioural_planner.STAY_STOPPED
            why_decelerate = bp._why_decelerate
            # obtain vehicle fence points for LP
            for i in range(len(vehicles)):
                x = vehicles[i][0]
                y = vehicles[i][1]
                z = vehicles[i][2]
                yaw = vehicles[i][3] + np.pi / 2.0  # add 90 degrees for fence
                spos = np.array([
                        [0, 0                       ],
                        [0, C4_VEHICLE_FENCELENGTH]])
                rotyaw = np.array([
                        [np.cos(yaw), np.sin(yaw)],
                        [-np.sin(yaw), np.cos(yaw)]])
                spos_shift = np.array([
                ...
                # When we stopped because of vehicles on the road, we check if
                # they are effectivelly obstructing our lane or not. If not, we move on.
                if stay_stopped and why_decelerate == 3:
                    # Compute the angle between the normalized vector between the vehicle
                    # and ego vehicle position with the ego vehicle's heading vector.
                    vehicle_delta_vector = [vehicles[i][0] - current_x, 
                                                vehicles[i][1] - current_y]
                    vehicle_distance = np.linalg.norm(vehicle_delta_vector)
                    # In this case, the vehicle is in the range of interest.
                    if vehicle_distance < 20:
                        vehicle_delta_vector = np.divide(vehicle_delta_vector, 
                                                        vehicle_distance)
                        ego_heading_vector = [math.cos(current_yaw), 
                                            math.sin(current_yaw)]
                        # Check to see if the relative angle between the vehicle and the ego
                        # vehicle lies within +/- 15 degrees of the ego vehicle's heading.
                        if np.dot(vehicle_delta_vector, 
                                ego_heading_vector) < ((math.sqrt(6) + math.sqrt(2))/4):
                            continue
                        vehicles_fences.append([spos[0,0], spos[1,0], spos[0,1], spos[1,1], vehicles[i][4]])
                else:
                    vehicles_fences.append([spos[0,0], spos[1,0], spos[0,1], spos[1,1], vehicles[i][4]])
            
            bp.set_vehicles_fences(vehicles_fences)
            ...
            for agent in measurement_data.non_player_agents:
                if agent.HasField('vehicle'):
                    lead_car_pos.append(
                            [agent.vehicle.transform.location.x,
                             agent.vehicle.transform.location.y,
                             math.radians(agent.vehicle.transform.rotation.yaw),
                             agent.id])
            ...
            if frame % LP_FREQUENCY_DIVISOR == 0:
                ...
                box = check_for_obstacles(sensor_data)

                depth=10000000
                if segmentation_data is not None and box is not None:
                    depth = compute_depth(segmentation_data, depth_data, box)
                    print("depth: ", depth)
                ...
                thr_pos = 30    # threshold on position chosed empirically
                for potential_lead_car in lead_car_pos:
                    # Yaw check - Checking if leading vehicle is in ego vehicle trajectory (i.e. it has the same ego orientation)
                    # and if potential lead vehicle is nearby ego vehicle.
                    if  (potential_lead_car[2] >= ego_state[2] - thr_yaw) and \
                        (potential_lead_car[2] <= ego_state[2] + thr_yaw) and \
                        (potential_lead_car[0] >= ego_state[0] - thr_pos) and \
                        (potential_lead_car[0] <= ego_state[0] + thr_pos) and \
                        (potential_lead_car[1] >= ego_state[1] - thr_pos) and \
                        (potential_lead_car[1] <= ego_state[1] + thr_pos):
                        if potential_lead_car[3] == id_car_ref:
                            bp._follow_lead_vehicle = True
                        bp.check_for_lead_vehicle(ego_state, potential_lead_car)
                        if potential_lead_car[3] == id_car_ref and bp._follow_lead_vehicle == False:
                            id_car_ref = -1
                        if bp._follow_lead_vehicle:
                            lead_car = potential_lead_car
                            index = lead_car_pos.index(potential_lead_car)
                            potential_lead_car_speed = lead_car_speed[index]
                            lead_car.append(potential_lead_car_speed)
                            id_car_ref = lead_car[3]
                            #print("ID: ", id_car_ref, "X_EGO: ", ego_state[0], "Y_EGO: ", ego_state[1], ". X_LEAD: ", lead_car[0], "Y_LEAD: ", lead_car[1], "EGO_YAW: ", ego_state[2], "LEAD_YAW: ", lead_car[2])
                            break
                ...
                lead_car_state = []
                if len(lead_car) > 0:
                    lead_car_state = [lead_car[0], lead_car[1], lead_car[4]]
